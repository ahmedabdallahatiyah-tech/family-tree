  ...
  for (const line of lines) {
    ...
    for (let level = 0; level < cells.length; level++) {
      ...
      if (level === 0) {
        ...
        if (!node) { node = { id: generateId(), name, children: [], depth: 1 }; root.children.push(node); }
        ancestors[0] = root; ancestors[1] = node; parent = node; parentDepth = 1;
        continue;
      }
      if (parent && parent.children) {
        let node = parent.children.find(c => c.name === name);
        if (!node) { node = { id: generateId(), name, children: [], depth: parentDepth + 1 }; parent.children.push(node); }
...
function getMaxDepth(node) {
  ...
  if (node.children) for (const c of node.children) max = Math.max(max, getMaxDepth(c));
  return max;
}
...
function filterTree(node, term) {
  ...
  const match = node.name.toLowerCase().includes(t);
  let filtered = [];
  if (node.children) for (const c of node.children) { const f = filterTree(c, term); if (f) filtered.push(f); }
  return (match || filtered.length > 0) ? { ...node, children: filtered } : null;
}

function getAllNames(node) {
  const names = [node.name];
  if (node.children) for (const c of node.children) names.push(...getAllNames(c));
  return names;
}
